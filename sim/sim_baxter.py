...
from robot import Arm, RobotGripper
import time
import math
import random
...
class Humanoid():
    def __init__(self, name_index=['',''], mode='velocity'):
        ...
        # the first arg adds the count like this:
        # suffix = '' if count == 0 else '#%d' % (count - 1)
        self.gripper_1 = JacoGripper(2)
        self.gripper_2 = JacoGripper(3)

class WamEnv():
    def __init__(self):
        self.pr = PyRep()
        # Launch the application with a scene file in headless mode
        self.pr.launch('../models/Baxter_test.ttt', headless=False)
        self.pr.start()  # Start the simulation
        self.collaborator= Humanoid(name_index=['#0',''], mode='ik')
        self.collaborator.gripper_1.actuate(0.0,0.4)
        for i in range(1000):
            self.step()
        self.collaborator.gripper_1.actuate(1.0,0.4)
        self.collaborator.gripper_2.actuate(0.0,0.4)
        for i in range(1000):
            self.step()
        self.collaborator.gripper_2.actuate(1.0,0.4)
